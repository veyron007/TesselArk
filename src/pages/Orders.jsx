...
export default function Orders({context={},refresh,onNavigate,onOpenInvoice,initialOrderId}) {
  ...
  return <div className="orders-page">
    <div className="orders-intro"><div><span className="orders-kicker">PROCUREMENT & SALES</span><h1>Orders and fulfillment</h1><p>Prepare quotations, confirm intent, then record partial receipts or dispatches against an order.</p></div><span className="orders-context">{context.bootstrap?.companies?.find(company=>String(company.id)===String(context.companyId))?.name} · {context.bootstrap?.companies?.find(company=>String(company.id)===String(context.companyId))?.gstins?.find(gstin=>String(gstin.id)===String(context.gstinId))?.gstin}</span></div>
    <div className="orders-boundary"><strong>Operational workflow</strong><span>Quotes, orders and physical movements do not create invoices, revenue, payables or GST credit. Accounting requires a separate approved invoice.</span></div>
    ...
    <div className="orders-tabs" role="tablist" aria-label="Order type">{[['sale','Sales orders'],['purchase','Purchase orders'],['quotes','Quotations']].map(([value,label])=><button role="tab" aria-selected={type===value} className={type===value?'active':''} key={value} onClick={()=>{setType(value);setSelectedId(null);setForm({number:'',partyId:'',orderDate:today(),notes:'',lines:[blankLine()]});}}>{label}</button>)}</div>
    {type==='quotes' ? <SalesQuotes context={context} refresh={refresh} onOpenOrder={(id)=>{setType('sale');setSelectedId(id);load();}} /> : <div className="orders-layout"><section className="orders-card"><div className="orders-card-head"><h2>{type==='sale'?'Sales':'Purchase'} order register</h2><span>{filtered.length} records</span></div>
