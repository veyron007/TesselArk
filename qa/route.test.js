...
test('workspace routes restore pages and supported source records from direct URLs', () => {
  ...
  assert.deepEqual(readWorkspaceRoute(new URL('http://localhost:3001/work-tasks?record=23&gstin=1&branch=2')), { page: 'work-tasks', recordId: 23 });
  ...
  assert.deepEqual(readWorkspaceRoute(new URL('http://localhost:3001/work-tasks?record=-2')), { page: 'work-tasks', recordId: null });
});
...
test('workspace URLs are bookmarkable and keep unrelated query settings', () => {
  ...
  assert.equal(workspaceUrl('work-tasks', 23, current, { gstinId: 1, branchId: 2 }), '/work-tasks?preview=1&record=23&gstin=1&branch=2');
